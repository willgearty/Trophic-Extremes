...
# plot limit
max_val <- max(c(abs(min(chng$perc_low)), abs(max(chng$perc_upp)))) * 1.01
...
traj <- ggplot(chng, aes(x = yr, y = perc_med, colour = diet_5cat)) +
  ...
  scale_y_continuous(name = "Percent change in mass", limits = c(-max_val, max_val)) +
...
# save plot
cowplot::save_plot("../figures/traj.pdf", traj_buff, base_height = 4, base_width = 9, dpi = 600)
