...
#lump insectivores in with carnivores
foss_mamm_diet_clean$Recoded_Diet[foss_mamm_diet_clean$Recoded_Diet == "insectivore"] <- "carnivore"
...
#diets separate
ggplot(foss_mamm_per_bin, aes(x = bin, y = lnMass_g, fill = factor(Recoded_Diet, levels = c("herbivore","omnivore","carnivore")))) +
  geom_violin(draw_quantiles = c(.25,.5,.75), position = position_dodge(), scale = "width") +
  scale_x_discrete(name = "Time (Ma)") +
  scale_y_continuous(name = "ln Mass (g)", lim = c(0,15)) +
  scale_fill_manual(values = colors3, name = "Diet") +
  theme_bw()
...
ggplot(foss_mamm_per_bin, aes(x = bin, y = lnMass_g, fill = archaic, color = factor(Recoded_Diet, levels = c("herbivore","omnivore","carnivore")))) +
  geom_boxplot(position = position_dodge(preserve = "single")) +
  scale_x_discrete(name = "Time (Ma)") +
  scale_y_continuous(name = "ln Mass (g)", lim = c(0,15)) +
...
diet_bootstrap <- data.frame(age_bin = rep(time_scale$name, each = n_subsets * n_diets, times = length(subset_sizes)),
                             diet = rep(diets, times = n_subsets * n_diets * length(subset_sizes)), sample = rep(subset_sizes, each = n_bins * n_diets * n_subsets),
                             mean = NA, sd = NA)
...
diet_differences$diets <- paste0(subset_differences$diet1, "-", subset_differences$diet2)
...
for(k in 1:length(subset_sizes)){
  ...
  for(j in 1:n_bins){
    ...
    for(i in 1:n_subsets){
      ...
      diet_differences$difference[idx:(idx + n_diets - 1)] <- sapply(1:n_diets, function(x) diffs[match(as.character(subset_differences$diet2[x]), rownames(diffs)),match(as.character(subset_differences$diet1[x]), colnames(diffs))])
      pw <- pairwise.wilcox.test(samp$lnMass_g, samp$Recoded_Diet)$p.value
      diet_differences$p[idx:(idx + n_diets - 1)] <- sapply(1:n_diets, function(x) pw[match(as.character(subset_differences$diet2[x]), rownames(pw)),match(as.character(subset_differences$diet1[x]), colnames(pw))])
...
ggplot(data = diet_bootstrap, aes(x = age_bin, y = mean, color = factor(diet, levels = c("herbivore", "omnivore", "carnivore")))) +
  geom_boxplot(position = position_dodge(preserve = "single")) +
  scale_x_discrete(name = "Time (Ma)", labels = gsub(" ","\n", rev(time_scale$name))) +
  scale_y_continuous(name = "ln Mass (g)", lim = c(0,15)) +
  theme_classic(base_size = 16) +
...
bootstrap_means <- diet_bootstrap %>%
  group_by(age_bin, diet, sample) %>%
  summarise(sd = sd(`mean`, na.rm = TRUE), mean = mean(mean), wtd_mean = wtd.mean(mean, 1/sd^2), wtd_var = wtd.var(mean, 1/sd^2))

ggplot(data = bootstrap_means, aes(x = age_bin, y = mean, color = diet, group = sample)) +
  geom_point(position = position_dodge(width = .9)) +
  geom_errorbar(aes(ymin = mean - 1.96*sd, ymax = mean + 1.96*sd), position = position_dodge(width = .9)) +
  scale_x_discrete(name = "Time (Ma)", labels = gsub(" ","\n", rev(time_scale$name))) +
  scale_y_continuous(name = "ln Mass (g)", lim = c(0,15)) +
  theme_classic(base_size = 16) +
  theme(axis.text = element_text(color = "black"), axis.text.x = element_text(color = "black", angle = 90, vjust = .5), axis.ticks = element_line(color = "black")) +
...
ggplot(data = weighted_means, aes(x = age_bin, y = wtd_mean, color = diet, group = sample)) +
  geom_point(position = position_dodge(width = .9)) +
...
ggplot(data = subset_differences, aes(x = age_bin, y = p, color = diets)) +
